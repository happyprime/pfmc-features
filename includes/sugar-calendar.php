 ...
 *
 * @param string $post_id The post ID of the Sugar Calendar event.
 * @return WP_Query The query for a post generated by the event.
 */
function generated_post_query( $post_id ) {
	$query = new \WP_Query(
		array(
			'meta_key'               => '_sc_event_id', // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
...
function generate_post( $post_id, $post ) {
	...
	// Return early if the nonce can't be verified.
	...
	$query = generated_post_query( $post_id );
...
function redirect_event_generated_posts() {
	...
	// Attempt to retrieve a value from the `_sc_event_id` meta key.
	$sc_event_id = get_post_meta( $post_id, '_sc_event_id', true );

	// Return early if the post has no `_sc_event_id` meta.
	if ( ! $sc_event_id ) {
		return;
	}

	// Redirect to the associated Sugar Calendar event post.
	wp_safe_redirect( esc_url( get_permalink( $sc_event_id ) ), 301 );
